...
export const markAssessmentComplete = async (req, res) => {
    try {
        ...
        console.log("Marking assessment as complete for:", { userId, moduleId });

        // Check if assessment already exists
        let assessment = await Assessment.findOne({ user: userId, module: moduleId });
...
export const getCompletedAssessments = async (req, res) => {
    try {
        ...
        res.status(200).json(completedAssessments.map(assessment => ({
            _id: assessment._id,
            user: assessment.user,
